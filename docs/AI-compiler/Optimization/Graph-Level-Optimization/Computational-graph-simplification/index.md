...
> **一句话定义**：计算图化简是 AI 编译器**高层图优化（Graph-Level Optimization）**的核心环节，指在**计算图（Computational Graph）**层面，通过一系列与硬件无关的变换，**删除冗余、合并算子、简化结构**，在不改变计算语义的前提下，减少计算量、内存访问和 kernel 启动开销。
...
图化简就是对这个 DAG 做**等价变换**，使其"更小、更快、更省内存"。
