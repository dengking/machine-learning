# GPU Computation & Neural Networks（GPU 计算与神经网络）

> 本文系统梳理 **GPU（Graphics Processing Unit，图形处理器）** 的计算特征，以及为什么以 **CNN（Convolutional Neural Network）** 和 **Transformer** 为代表的现代神经网络能与 GPU / **Tensor Core** 流水线高度契合，而 **Capsule Network（胶囊网络）** 的动态路由（Dynamic Routing）却难以在 GPU 上高效执行。
> 
> 参考资料：
> 
> - NVIDIA CUDA C++ Programming Guide：<https://docs.nvidia.com/cuda/cuda-c-programming-guide/>
> - NVIDIA GPU Architecture Whitepapers（Volta / Ampere / Hopper）：<https://www.nvidia.com/en-us/data-center/resources/>
...
# 第一部分：GPU 擅长的计算（What GPU is Good At）
...
- **Warp Divergence（线程分裂 / 线程束发散）**：同一 warp 内的 32 个线程若走入不同分支，硬件需串行执行两条路径，有效算力减半甚至更多。参考：<https://developer.nvidia.com/blog/using-cuda-warp-level-primitives/>
- **Memory Coalescing（合并访存）**：同一 warp 内的线程若访问连续显存地址，可合并为一次内存事务，带宽利用率最高。参考：CUDA Best Practices Guide §9.2 *Coalesced Access to Global Memory*。
- **Occupancy（占用率）**：SM（Streaming Multiprocessor，流式多处理器）上活跃 warp 数与最大可容纳 warp 数之比，占用率高才能通过 warp 切换掩盖访存延迟。

---

## 三、GPU 擅长的典型计算类型

### 3.1 矩阵 / 向量运算（General Matrix Multiplication, GEMM）⭐
...
> 一个 $1000\times1000$ 的矩阵乘法有 100 万个独立的输出元素，GPU 可让数千线程同时开算。实际实现会使用 **分块矩阵乘（Tiled GEMM）**，将大矩阵划分为适配 Shared Memory 的小块，兼顾并行度与数据复用（data reuse）。

### 3.2 逐元素运算（Element-wise Operations）
...
回到前面的例子 $y = x_2 \cdot \sin(x_1)$：如果 $x_1, x_2$ 是**百万维向量**，GPU 可让百万线程同时计算每个分量的 $\sin$ 和乘法。
...
> 📌 **核心洞察**：GPU 与 CPU 不是谁强谁弱，而是**分工不同（Heterogeneous Computing，异构计算）**——CPU 追求"低延迟处理复杂串行任务"，GPU 追求"高吞吐处理海量并行任务"。深度学习之所以在 GPU 上爆发，正是因为神经网络的前向传播与反向传播（reverse-mode autodiff）**本质上就是可高度并行的大规模矩阵运算**。理解这一点，就理解了为什么"算力（compute）"在 AI 时代如此关键——它直接决定了我们能训练多大的模型、处理多少数据（*Scaling Laws*，参考 Kaplan et al. 2020: <https://arxiv.org/abs/2001.08361>）。反过来，如果你的算法充满串行依赖和复杂分支（如某些图算法、递归逻辑、动态路由），GPU 未必是最优选择，这时 CPU、FPGA 或专用 ASIC（如 Google TPU、Graphcore IPU、Cerebras WSE）可能更合适。

---

# 第二部分：CNN、Transformer 为何适配 GPU / Tensor Core 流水线

先铺垫 GPU 底层核心逻辑：**GPU 天生擅长「大批量、同一种计算、顺序流水、无判断分支」的矩阵乘法**。**Tensor Core** 是 NVIDIA 从 Volta 架构（V100，2017）开始引入的专用矩阵乘累加（MMA, Matrix Multiply-Accumulate）硬件单元，专为加速低精度矩阵乘（FP16 / BF16 / TF32 / FP8 / INT8）设计；一旦出现循环、if 分支、不规则访存，硬件利用率会暴跌。
...
## 一、三个关键词：无分支、单向、批量矩阵乘

### 1. 批量矩阵乘（Batch Matrix Multiplication, BMM / MatMul）

CNN 卷积、Transformer 自注意力（Self-Attention）的核心数学本质，都可以等价转化为 **稠密批量矩阵乘法（Dense Batched GEMM）**：

1. **CNN 卷积（Convolution）**
   卷积通过 **im2col**（image-to-column，图像展开）把滑动窗口变成二维矩阵，`输出特征图 = 卷积核矩阵 × 图像窗口矩阵`；
   一次前向传播就是连续多层大矩阵相乘，所有样本（batch）、所有通道（channel）并行批量计算。
...
**Tensor Core 的设计目标就是加速这类大规模矩阵乘**：硬件内置矩阵乘专用运算单元，一次吞吐 16×16×16 / 32×8×16 等小块矩阵（对应 PTX 指令 `mma.sync` / WMMA API），FP16/BF16 算力比通用 CUDA Core 高 10 倍以上。只要算子是标准 MatMul，**cuDNN**、**cuBLAS**、**CUTLASS**、**TensorRT** 会自动将计算调度给 Tensor Core，硬件跑满。

- CUTLASS（NVIDIA 开源的 CUDA 模板矩阵运算库）：<https://github.com/NVIDIA/cutlass>
- TensorRT：<https://developer.nvidia.com/tensorrt>

反观 **Capsule Network** 的 **动态路由（Dynamic Routing-by-Agreement）**：
核心是**迭代投票循环**，每轮要单独计算耦合系数（coupling coefficients）$c_{ij}$、加权求和、Softmax 归一化，不是整块批量矩阵乘，矩阵尺寸零散、计算碎片化，Tensor Core 无法生效，只能退化到低效通用 CUDA Core。

- Sabour, Frosst, Hinton, *"Dynamic Routing Between Capsules"*, NeurIPS 2017. <https://arxiv.org/abs/1710.09829>

### 2. 单向（Feed-Forward，前馈无循环、无数据回流依赖）

CNN / MLP（Multi-Layer Perceptron）/ Transformer 都是**严格前向数据流（strict feed-forward dataflow）**：

- 第 $l$ 层计算只依赖第 $l-1$ 层输出；
- 计算完一层即可丢弃中间临时缓存（activation 除非需要用于反传，否则可释放）；数据只向前流动，不存在"这一轮结果要传回上一层重复计算"；
- 数据流固定、计算顺序固定，GPU 可以做 **软件流水线预取（Software Pipelining / Prefetching）**：计算当前层的同时，通过 **异步拷贝（`cp.async`，Ampere+）** 提前把下一层权重 / 特征从 **HBM（High-Bandwidth Memory，高带宽显存）** 加载到片上共享内存（Shared Memory）/ 寄存器（Register File），掩盖访存延迟（Latency Hiding）。

> **HBM** 是 GPU 显存的核心技术：A100 使用 HBM2e（带宽 1.6–2.0 TB/s），H100 使用 HBM3（3 TB/s）。参考：<https://en.wikipedia.org/wiki/High_Bandwidth_Memory>

**Capsule 动态路由是循环迭代依赖（Iterative Dependency）**（原生论文迭代 3 次）：
第 $t$ 轮耦合系数 $c_{ij}^{(t)}$ 完全依赖第 $t-1$ 轮胶囊输出，必须等上一轮全部算完才能启动下一轮，数据来回读写，GPU 流水线无法预取并行，等待空转时间大幅增加，**显存带宽瓶颈（memory-bandwidth-bound）被放大**。

> 类似的循环依赖问题也出现在 **RNN / LSTM** 的时间步（timestep）展开中——这也是为什么 Transformer 取代 RNN 成为大模型主流的一个重要 *硬件层面* 原因。

### 3. 无分支（No Branching，无 if 判断、无动态分支跳转）

GPU 是 **SIMT（Single Instruction, Multiple Threads，单指令多线程）** 架构：同一个 warp（32 线程为一组）内所有线程必须执行完全相同的指令；一旦某个线程触发 `if / else` 分支，GPU 会串行走完两条分支（**Warp Divergence，线程束发散 / 线程分裂**），算力直接腰斩甚至更多。

- CUDA C++ Programming Guide §5.4.2：<https://docs.nvidia.com/cuda/cuda-c-programming-guide/index.html#simt-architecture>

**CNN / Transformer** 每层运算逻辑统一：全部是矩阵乘 + 逐元素激活（**ReLU** / **GELU** / **Softmax** / **LayerNorm**），**没有条件判断、没有动态分支**；所有样本、通道执行一模一样的计算流程，warp 不会分裂，硬件接近 100% 利用率。

**Capsule 动态路由**包含大量条件逻辑：迭代次数循环、耦合系数加权筛选、**Squash 函数**归一化更新，大量分支判断，大量线程走不同计算路径，warp 分裂严重，流水线频繁中断。

## 二、什么是 GPU / Tensor Core 流水线？
